...
class MongoDB:
    ...
    def update(self, data, sets, upsert=False, ):
        try:
            # {'$set': data}
            if self.post.update(data, sets, upsert = upsert)[
                    'updatedExisting']:
                return 'exists'
            else:
                return 'success'
    ...
    def update_many(self, data, upsert=False):
        try:
            self.post.update_many(data, data, upsert=upsert)
            return True
        except BaseException:
            return False

    def find(self, select, limit=False, **kwargs):
        try:
            if 'projection' in kwargs:
                projection = kwargs['projection']
                if limit:
                    return self.post.find(select,projection).limit(limit)
                else:
                    return self.post.find(select,projection)
            else:
                if limit:
